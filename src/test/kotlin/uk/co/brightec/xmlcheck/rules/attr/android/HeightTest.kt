...
import uk.co.brightec.xmlcheck.rules.attr.android.Height.Companion.RULE_HEIGHT_2S
import uk.co.brightec.xmlcheck.testutil.Small

@Small
private class HeightTest {

    private lateinit var check: Height
    ...
    @Test
    fun `Given Height2s suppressed | When run check | Then no failure`() {
        // GIVEN
    ...
    fun `Given value not in 2s | When run check | Then failure`(value: String) {
        ...
        // THEN
        val expected = RULE_HEIGHT_2S.failure(node)
